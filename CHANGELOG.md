...
- **PyPI-first configuration model.** The bridge is now published to PyPI
  ...
  longer assumes a git checkout with `.venv/` and `deploy/…plist`. The
  config surface is now a **bridge binary path** (default
  `~/.local/bin/opencode-talk-bridge`) and a **config/working directory**
  (default `~/.config/opencode-talk-bridge`, holding `.env`, `status.json`,
  `bridge.sqlite3`). Settings keys changed from `bridgeRepoPath` to
  `bridgeBinaryPath` + `configDirPath`; existing installs fall back to the
  new defaults and re-confirm via onboarding.
- The launchd plist is now **generated from scratch** (`PlistTemplate.generate`)
  rather than read+rewritten from the repo's `deploy/…plist` — the PyPI wheel
  doesn't ship one. `installPlist()` also creates the config dir so launchd's
  `WorkingDirectory` is valid.
- Onboarding and the Bridge settings tab now take a binary file path + a
  config directory (with a new `PathPicker.pickFile`), show binary /
  config-dir / `.env` health, and offer **Konfig-Ordner anlegen**.
  `BridgeRepoValidator` → `BridgeSetupValidator` (`isUsable` = an executable
  bridge binary; the config dir is auto-created on install).

### Added
- App icon: a teal→indigo squircle with the radiowaves motif, generated by
  `Scripts/make-icon.{py,sh}` into `Resources/AppIcon-candidate.png`,
  `AppIcon.iconset/`, and `AppIcon.icns`, wired via `CFBundleIconFile`.
- "Dienst entfernen…" button in Settings → Bridge: confirms, then
  `bootout`s the launchd job and deletes the installed plist. Install and
  uninstall now force-refresh the service-state poller immediately.
- Notarization-ready release pipeline. `Scripts/sign.sh` honours
  `SIGN_IDENTITY` (default `-` = ad-hoc) and `Scripts/notarize.sh` is a
  no-op until `APPLE_NOTARY_*` credentials are present, so switching on a
  Developer ID + notarization later needs only secrets, not a pipeline
  rewrite. Replaces `sign-adhoc.sh`.
